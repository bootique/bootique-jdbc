...
There are more options that you can use when creating a DbTester, but since they are the same for either Derby or
Testcontainers DB, we'll look at them in a <<dbtester-db-initialization,common section below>>.

=== DB started on Docker
...
The rest of the examples will use `TcDbTester`, but they should work the same way for Derby.
...
Since the DB may be setup once and reused between multiple tests, it is often important to reset it to a known state
...
violations during delete, you must pass tables names to this method in the "insert" order (i.e. if "t2" has an FK
