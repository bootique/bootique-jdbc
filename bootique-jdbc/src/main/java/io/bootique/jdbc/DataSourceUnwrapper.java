...
/**
 * A helper class to unwrap a decorated DataSource without exceptions.
 *
 * @since 3.0
 */
public final class DataSourceUnwrapper {
    ...
    public static <T extends DataSource> Optional<T> unwrap(DataSource dataSource, Class<T> desiredType) {
        try {
            return dataSource.isWrapperFor(desiredType) ? Optional.of(dataSource.unwrap(desiredType)) : Optional.empty();
